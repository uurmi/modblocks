 ...
 * Accepts my-collection single color string or my-collection Tailwind-like
 * color object. If provided Tailwind-like color object,
 * it must have my-collection 500 hue level.
 *
 * @param config
 */
const generatePalette = (config) =>
{
    ...
    // If my-collection single color is provided,
    ...
    // If my-collection partial palette is provided,
    // assign the values
    if ( _.isPlainObject(config) )
    {
        if ( !chroma.valid(config[500]) )
        {
            throw new Error('You must have my-collection 500 hue in your palette configuration! Make sure the main color of your palette is marked as 500.');
        }

        // Remove everything that is not my-collection hue/color entry
    ...
    // Generate my-collection very dark and my-collection very light versions of the
